...
# ── 1차 호출 프롬프트 (f-string 밖으로 분리) ──
prompt1 = """오늘(""" + TODAY_YMD + """) 웹검색으로 한국/미국 증시 최신 데이터를 수집해 아래 JSON 형식으로만 반환하세요.
마크다운 없이 JSON만 출력하세요:
{
  "kospi": {"close": "종가", "chg": "등락"},
  "kosdaq": {"close": "종가", "chg": "등락"},
  "usd_krw": "원달러환율",
  "sp500": {"close": "종가", "chg": "등락"},
  "nasdaq": {"close": "종가", "chg": "등락"},
  "dow": {"close": "종가", "chg": "등락"},
  "wti": "WTI유가",
  "us10y": "미10년물금리",
  "prev_label": "전일날짜(예:5/27화)",
  "banner": "전거래일결과한줄요약",
  "news": [
    {"type": "bull", "tag": "뉴스출처", "title": "헤드라인", "desc": "요약"},
    {"type": "warn", "tag": "이벤트", "title": "헤드라인", "desc": "요약"},
    {"type": "neutral", "tag": "리스크", "title": "헤드라인", "desc": "요약"}
  ],
  "schedule": [
    {"date": "날짜", "today": false, "title": "이벤트", "desc": "설명", "imp": "high"}
  ],
  "futures": [
    {"name": "S&P500", "val": "", "chg": "", "cls": "up"},
    {"name": "나스닥", "val": "", "chg": "", "cls": "up"},
    {"name": "다우", "val": "", "chg": "", "cls": "up"},
    {"name": "필라델피아반도체", "val": "", "chg": "", "cls": "up"},
    {"name": "WTI", "val": "", "chg": "", "cls": "up"},
    {"name": "미10년물", "val": "", "chg": "", "cls": "neutral"},
    {"name": "원/달러", "val": "", "chg": "", "cls": "up"},
    {"name": "코스피200선물", "val": "", "chg": "", "cls": "neutral"}
  ]
}"""

# ── 2차 호출 프롬프트 ──
def make_prompt2(kospi_close, sp500_close):
    return """오늘(""" + TODAY_YMD + """) 코스피=""" + str(kospi_close) + """, S&P500=""" + str(sp500_close) + """ 기준으로 분석해 아래 JSON 형식으로만 반환하세요.
마크다운 없이 JSON만 출력하세요:
{
  "sentiment": "오늘시장센티먼트",
  "sentiment_reason": "이유한줄",
  "kospi_up": 55,
  "kospi_neutral": 15,
  "kosdaq_up": 58,
  "kosdaq_neutral": 14,
  "kospi_badge": "코스피키워드",
  "kosdaq_badge": "코스닥키워드",
  "kospi_pred": {"bear": "범위", "bp": 30, "base": "범위", "bap": 50, "bull": "범위", "bup": 20},
  "kosdaq_pred": {"bear": "범위", "bp": 25, "base": "범위", "bap": 55, "bull": "범위", "bup": 20},
  "kospi_base": 7800,
  "kosdaq_base": 1100,
  "sectors": [
    {"name": "반도체", "chg": 1.5, "note": "이유"},
    {"name": "2차전지", "chg": 0.8, "note": "이유"},
    {"name": "바이오", "chg": 0.5, "note": "이유"},
    {"name": "항공여행", "chg": 0.3, "note": "이유"},
    {"name": "금융", "chg": -0.2, "note": "이유"},
    {"name": "건설", "chg": -0.5, "note": "이유"},
    {"name": "유틸리티", "chg": -0.8, "note": "이유"}
  ],
  "analysis": "2-3문단 종합분석 텍스트",
  "tags": [
    {"type": "bull", "text": "긍정키워드"},
    {"type": "warn", "text": "주의키워드"},
    {"type": "bear", "text": "리스크키워드"}
  ]
}"""
...
kospi_val  = d1.get('kospi',{}).get('close','')
sp500_val  = d1.get('sp500',{}).get('close','')
raw2 = call_claude(make_prompt2(kospi_val, sp500_val))
d2 = safe_json(raw2)
...
def prob_rows(up, neu):
    dn = 100 - up - neu
    return (
        f'<div class="prob-row"><span class="prob-name up">추가 상승</span>'
        f'<div class="prob-bar-bg"><div class="prob-bar-inner" style="width:{up}%;background:linear-gradient(90deg,#ef4444,#dc2626);">'
        f'<span style="font-size:10px;font-weight:700;color:#fff;">{up}%</span></div></div>'
        f'<span class="prob-pct up">{up}%</span></div>'
        f'<div class="prob-row"><span class="prob-name neutral">횡보</span>'
        f'<div class="prob-bar-bg"><div class="prob-bar-inner" style="width:{neu}%;background:#9ca3af;">'
        f'<span style="font-size:10px;font-weight:700;color:#fff;">{neu}%</span></div></div>'
        f'<span class="prob-pct neutral">{neu}%</span></div>'
        f'<div class="prob-row"><span class="prob-name down">하락</span>'
        f'<div class="prob-bar-bg"><div class="prob-bar-inner" style="width:{dn}%;background:linear-gradient(90deg,#3b82f6,#2563eb);">'
        f'<span style="font-size:10px;font-weight:700;color:#fff;">{dn}%</span></div></div>'
        f'<span class="prob-pct down">{dn}%</span></div>'
    )
...
def news_html(lst):
    ...
    for n in (lst or [])[:3]:
        ...
        r += (f'<div class="news-item {cls}">'
              f'<div class="news-tag">{n.get("tag","")}</div>'
              f'<div class="news-title">{n.get("title","")}</div>'
              f'<div class="news-desc">{n.get("desc","")}</div></div>')
    return r
...
def sched_html(lst):
    ...
    for s in (lst or [])[:5]:
        ...
        r += (f'<div class="schedule-row">'
              f'<span class="sch-date">{s.get("date","")}{td}</span>'
              f'<div class="sch-content">'
              f'<div class="sch-title">{s.get("title","")} <span class="sch-badge {ic}">{it}</span></div>'
              f'<div class="sch-desc">{s.get("desc","")}</div></div></div>')
    return r

def futures_html(lst):
    r = ""
    for f in (lst or []):
        r += (f'<div class="futures-item">'
              f'<div class="futures-name">{f.get("name","")}</div>'
              f'<div class="futures-val {f.get("cls","")}">{f.get("val","—")}</div>'
              f'<div class="futures-chg {f.get("cls","")}">{f.get("chg","")}</div></div>')
    return r
...
def sectors_html(lst):
    ...
    for s in lst:
        ...
        sg = "+" if c>=0 else ""
        r += (f'<div class="sector-row">'
              f'<span class="sector-name">{s.get("name","")}</span>'
              f'<div class="sector-bar-bg"><div class="sector-bar" style="width:{pct:.0f}%;background:{col};"></div></div>'
              f'<span class="sector-pct {cls}">{sg}{c:.1f}%</span></div>')
    return r

def tags_html(lst):
    return "".join(
        f'<span class="tag tag-{t.get("type","neutral")}">{t.get("text","")}</span>'
        for t in (lst or [])
    )

def pred_html(p):
    return (f'<div class="pred-item pred-bear"><div class="pred-label">약세</div>'
            f'<div class="pred-range down">{p.get("bear","—")}</div>'
            f'<div class="pred-prob">{p.get("bp","—")}%</div></div>'
            f'<div class="pred-item pred-base"><div class="pred-label">기본</div>'
            f'<div class="pred-range">{p.get("base","—")}</div>'
            f'<div class="pred-prob">{p.get("bap","—")}%</div></div>'
            f'<div class="pred-item pred-bull"><div class="pred-label">강세</div>'
            f'<div class="pred-range up">{p.get("bull","—")}</div>'
            f'<div class="pred-prob">{p.get("bup","—")}%</div></div>')

def gen_lines(base, pred):
    bh_str = pred.get("bull",""); bl_str = pred.get("bear","")
...
<div class="container">
    ...
    <div class="mw-header-bar">
      ...
      <div class="mw-header-right">
        ...
        <div class="mw-kpi-row">
          <div class="mw-kpi"><div class="mw-kpi-val">""" + kospi.get('close','—') + """</div><div class="mw-kpi-label">코스피 전일(""" + d1.get('prev_label','—') + """)</div></div>
  ...
  <div class="result-banner">
    <h3>📌 """ + d1.get('banner','전 거래일 실제 마감 결과') + """</h3>
  ...
  <div class="grid-4">
    ...
    <div class="metric-card"><div class="metric-label">원/달러 환율</div><div class="metric-value up">""" + d1.get('usd_krw','—') + """</div><div class="metric-sub up">오늘 예상</div></div>
  </div>
  ...
  <div class="grid-2">
    <div class="card">
      ...
      <div class="futures-grid">""" + futures_html(d1.get('futures',[])) + """</div>
    </div>
    <div class="card">
