...
def call_claude_pure(prompt):
    """웹검색 없음 - 분석/전망 생성용 (토큰 절약, 안정적)"""
    response = client.messages.create(
        model="claude-haiku-4-5",
        max_tokens=2000,
        messages=[{"role": "user", "content": prompt}]
...
def pct(c, p): return round((c-p)/p*100, 2) if c and p else None
...
def make_banner():
    parts = []
    if kospi_chg_pct: parts.append(f"코스피 {kospi_chg_pct:+.2f}%")
...
r2 = safe_json(call_claude_search(
    "오늘 " + TODAY_YMD + " 한국 증시 최신 뉴스와 이번주 경제일정을 웹검색해서 JSON만 반환:\n"
    '{"news":[{"type":"bull","tag":"🇺🇸 미국 증시 (' + PREV_SHORT + ')","title":"헤드라인","desc":"2-3문장"},'
    '{"type":"warn","tag":"📅 이번주 핵심 이벤트","title":"헤드라인","desc":"2-3문장"},'
    '{"type":"neutral","tag":"⚠️ 주요 리스크","title":"헤드라인","desc":"2-3문장"}],'
    '"us_market_comments":{"sp500":"코멘트","nasdaq":"코멘트","dow":"코멘트","semicon":"코멘트","wti":"코멘트","us10y":"코멘트","usd_krw":"코멘트","kospi200f":"코멘트"},'
    '"schedule":[{"date":"날짜","today":false,"title":"이벤트","desc":"설명","imp":"high"},'
...
k_val  = fmt(kospi_close)  if kospi_close  else "8000"
kp_val = f"{kospi_chg_pct:+.2f}%" if kospi_chg_pct else "+0.00%"
kd_val = fmt(kosdaq_close) if kosdaq_close else "1100"
sp_val = fmt(sp500_close)  if sp500_close  else "5000"
kb_price = int(kospi_close)  if kospi_close  else 8000
kdb_price= int(kosdaq_close) if kosdaq_close else 1100

r3 = safe_json(call_claude_pure(
    "오늘 " + TODAY_YMD + " 한국 증시 전망 분석. 전일 확정 데이터: 코스피=" + k_val + "(" + kp_val + "), 코스닥=" + kd_val + ", S&P500=" + sp_val + "\n"
    "아래 JSON 형식으로만 반환 (마크다운 없이 JSON만):\n"
    '{"sentiment":"오늘장세한마디",'
    '"sentiment_reason":"근거한줄",'
    '"kospi_up":58,"kospi_neutral":16,'
    '"kosdaq_up":63,"kosdaq_neutral":15,'
    '"kospi_badge":"키워드","kosdaq_badge":"키워드",'
    '"kospi_bear":"' + str(kb_price-150) + '~' + str(kb_price-50) + '",'
    '"kospi_bear_prob":30,'
    '"kospi_base":"' + str(kb_price-50) + '~' + str(kb_price+100) + '",'
    '"kospi_base_prob":52,'
    '"kospi_bull":"' + str(kb_price+100) + '~' + str(kb_price+200) + '",'
    '"kospi_bull_prob":18,'
    '"kosdaq_bear":"' + str(kdb_price-30) + '~' + str(kdb_price-10) + '",'
    '"kosdaq_bear_prob":25,'
    '"kosdaq_base":"' + str(kdb_price-10) + '~' + str(kdb_price+20) + '",'
    '"kosdaq_base_prob":55,'
    '"kosdaq_bull":"' + str(kdb_price+20) + '~' + str(kdb_price+50) + '",'
    '"kosdaq_bull_prob":20,'
    '"kospi_base_price":' + str(kb_price) + ','
    '"kosdaq_base_price":' + str(kdb_price) + ','
    ...
    '"analysis":"오늘 장세 2-3문단 분석 HTML strong 태그 사용 가능",'
...
us_market = [
    {"name":"S&P 500",         "val":fmt(sp500_close),       "chg":(f"{sp500_chg_pct:+.2f}% · " if sp500_chg_pct else "")+cmts.get("sp500",""),  "cls":"up" if (sp500_chg_pt or 0)>=0 else "down"},
    {"name":"나스닥",           "val":fmt(nasdaq_close),      "chg":(f"{nasdaq_chg_pct:+.2f}% · " if nasdaq_chg_pct else "")+cmts.get("nasdaq",""),"cls":"up" if (nasdaq_chg_pt or 0)>=0 else "down"},
    {"name":"다우존스",         "val":fmt(dow_close),         "chg":(f"{dow_chg_pct:+.2f}% · " if dow_chg_pct else "")+cmts.get("dow",""),        "cls":"up" if (dow_chg_pt or 0)>=0 else "down"},
...
k_up  = int(d.get('kospi_up',  58)); k_n  = int(d.get('kospi_neutral', 16))
kd_up = int(d.get('kosdaq_up', 63)); kd_n = int(d.get('kosdaq_neutral',15))
...
kb,kh,kl   = gen_chart(d.get('kospi_base_price',  kb_price),  d.get('kospi_bear',''),  d.get('kospi_bull',''))
kdb,kdh,kdl= gen_chart(d.get('kosdaq_base_price', kdb_price), d.get('kosdaq_bear',''), d.get('kosdaq_bull',''))
...
<div class="container">
  ...
  <div class="grid-2">
    <div class="card">
      ...
      <div class="pred-band">
        <div class="pred-item pred-bear"><div class="pred-label">약세</div><div class="pred-range down">""" + str(d.get('kospi_bear','—')) + """</div><div class="pred-prob">""" + str(d.get('kospi_bear_prob','—')) + """%</div></div>
        <div class="pred-item pred-base"><div class="pred-label">기본</div><div class="pred-range">""" + str(d.get('kospi_base','—')) + """</div><div class="pred-prob">""" + str(d.get('kospi_base_prob','—')) + """%</div></div>
        <div class="pred-item pred-bull"><div class="pred-label">강세</div><div class="pred-range up">""" + str(d.get('kospi_bull','—')) + """</div><div class="pred-prob">""" + str(d.get('kospi_bull_prob','—')) + """%</div></div>
      </div>
    ...
    <div class="card">
      ...
      <div class="pred-band">
        <div class="pred-item pred-bear"><div class="pred-label">약세</div><div class="pred-range down">""" + str(d.get('kosdaq_bear','—')) + """</div><div class="pred-prob">""" + str(d.get('kosdaq_bear_prob','—')) + """%</div></div>
        <div class="pred-item pred-base"><div class="pred-label">기본</div><div class="pred-range">""" + str(d.get('kosdaq_base','—')) + """</div><div class="pred-prob">""" + str(d.get('kosdaq_base_prob','—')) + """%</div></div>
        <div class="pred-item pred-bull"><div class="pred-label">강세</div><div class="pred-range up">""" + str(d.get('kosdaq_bull','—')) + """</div><div class="pred-prob">""" + str(d.get('kosdaq_bull_prob','—')) + """%</div></div>
      </div>
